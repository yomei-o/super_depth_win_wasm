# 引継ぎ（2026-09-05 未明 — Bio ロゴまで動く）

この節を最初に読めば続きから入れる。**移植は始まっている**:
インストーラを解体し、Ghidra で読める土台を作り、状態機械の入口
（0x0a → 0x0f/0x10 の Bio_100% ロゴ）が原作どおりに動く。
...
6. **状態 0x1e（海のタイトル）**。空・海面・海中のタイルの敷き方と、
   7 レーンの乱数スポーン。`out/superdepth.c` の case 0x1e（3810 行から）
