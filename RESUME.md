# 引継ぎ（2026-09-05 朝 — 遊べる）

この節を最初に読めば続きから入れる。**ゲーム本体まで動く**:
インストーラを解体し、Ghidra で読める土台を作り、
**Bio ロゴ（0x10）→ 海のタイトル（0x1e）→ ゲーム本体（0x32）**が
原作どおりに動く。爆雷で潜水艦を沈められる。
...
9. ~~ゲーム本体~~ 済み（`src/play.c`、`tests/play_check.c`）。
   残りは次の 3 つ:
   * **面クリア** `FUN_00408210`（海面まで上がる演出 → `FUN_0040c9e0`）
   * **ゲームオーバーと名前入力** `FUN_0040bdb0`（`rankin = %d rcurX`）
   * **得点のポップアップ** `FUN_0040b740` と `FUN_0040b6c0`、
     ポーズ画面 `FUN_0040b960`
