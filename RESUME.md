...
9. **残っているのはこれだけ**:
   * エンディング `FUN_00408650`（120 行、ending.dar と bgm14）。
     **この build では面クリアからは入れない** —— 0x426100 からの
     面選び debug 分岐だけが入口なので、そこも一緒に読む必要がある
   * SOUND TEST（0x46）も同じで、`FUN_00421da0(0x46)` を呼ぶ所が無い
   * `FUN_00425f80` の中の面選び（Ghidra が関数と見なしていない
     0x426100..0x4266d1）。何のキーで出るのかは未確認
