...
* **面は 3 つ動く**: 海（`src/play.c`）→ 空（`src/air.c`）→
  宇宙（`src/space.c`、台本は `stage3.bin`）。4 面はまだ
...
9. **次の一手**: `FUN_00403dc0`（4 面、917 行、`LAB_004011c7`）。
   space.dar・敵 10・入口で白フラッシュ 6。宇宙の面の次に入る。
   その先にエンディング `FUN_00408650`（120 行、ending.dar と bgm14。
   **この build では面クリアから入れない**）
...
* **4 面 = `FUN_00403dc0`（917 行、`LAB_004011c7`）**。space.dar、敵 10、
  入口で `DAT_0046217c = 6`（白フラッシュ）。宇宙の面の次に入る
