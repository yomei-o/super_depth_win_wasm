...
**原作の画面はぜんぶ動く。** ロゴ → タイトル → 海 → 空 → 宇宙 →
大物 → また海 …。沈められればゲームオーバー → 名前入力 → 得点表 →
タイトル。**エンディング → CAST → スタッフロール**も動く（原作と同じく
デバッグ メニューからしか出られない —— 下記）。

**いちばん強い裏付け**: 原作の録画 `disk/demo1.dat`（8235 フレームの
入力記録）を**最後まで再生できる** —— 海 → 空 → 宇宙 → 大物 と進み、
沈められて終わるのではなく記録切れで終わる。乱数の順・敵の動き・
当たり判定・アイテムの出方が噛み合わないと通らない検査で、
`tmp/play_check.exe` が毎回見ている。
